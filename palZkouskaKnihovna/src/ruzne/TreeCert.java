...
public class TreeCert {
	...
	// je potřeba mu dát graf kde je horní diagonala symetricka ze spodní
	// tedy array[i][j] = value a array[j][i] = value
	boolean[][] graphA;
	...
	public TreeCert(boolean[][] graphA) {
		this.graphA = graphA;
		certs = new ArrayList[graphA.length];
		this.closed = new boolean[graphA.length];
		this.onlyLeaves = new boolean[graphA.length];
		initCerts();
	}
